...
class VerEvaluacionDialog(QDialog):
    ...
    def __init__(self, alumno_id: uuid.UUID, parent=None):
        ...
        self.servicio = EvaluacionService(self.sessions)
    ...
    def _cargar_evaluaciones(self):
        try:
            self.evaluaciones = self.servicio.listar_evaluaciones(self.alumno_id)
        except Exception:
            logger.exception("Error al cargar evaluaciones")
            QMessageBox.critical(self, "Error", "No se pudieron cargar las evaluaciones")
            self.evaluaciones = []
    ...
    #     print("editar actual")
        ...
        # if evaluacion_id is None:
        #     return
        # from app.ui.dialogs.editar_evaluacion_dialog import EditarEvaluacionDialog
        # dialog = EditarEvaluacionDialog(evaluacion_id=evaluacion_id, alumno_id=self.alumno_id, parent=self)
        # if dialog.exec():
        #     self._cargar_evaluaciones()
    ...
    def _eliminar_actual(self):
        ...
        if confirm == QMessageBox.StandardButton.Yes:
            try:
                self.servicio.eliminar_evaluacion(evaluacion_id)
                self._cargar_evaluaciones()
            except Exception:
